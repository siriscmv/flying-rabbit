...
(function addElements() {
  ...
  internals.scene.add(mainObject.mesh);
...
window.addEventListener('keydown', (event) => {
  ...
  else if (event.shiftKey) x = 100
  else if (event.ctrlKey) x = -100

  lastTween = TweenMax.to(mainObject.mesh.position, 10, {
    x,
    y,
    z,
    onUpdate: () => {
      x = mainObject.mesh.position.x;
      y = mainObject.mesh.position.y;
      z = mainObject.mesh.position.z;
    },
  }); 
});
